...
const ShelfClient = ({ userId, nickname, playlists, groups }: Props) => {
  ...
  const handleDeletePlaylist = (id: string) => {
    ...
    startTransition(async () => { await deletePlaylist(id); });
  };

  const handleLeaveGroup = (groupId: string) => {
    if (!confirm("그룹에서 나갈까요?")) return;
    startTransition(async () => { await leaveGroup(groupId); });
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="p-6 pb-0">
        ...
        <p className="text-muted-foreground text-sm mt-1">안녕하세요, {nickname} 님</p>
      </header>
                      ...
                      <CardHeader>
                        <div className="flex items-center justify-between gap-2">
                          <Link href={`/playlist/${playlist.id}`} className="flex-1 min-w-0">
                            ...
                            <CardDescription>{playlist.Track?.[0]?.count ?? 0}곡</CardDescription>
                          </Link>
                          <div className="flex gap-1 shrink-0">
                            <Button variant="ghost" size="icon-sm" onClick={() => setEditTarget(playlist)}>
                              <HugeiconsIcon icon={Edit01Icon} strokeWidth={2} />
                            </Button>
                            <Button variant="ghost" size="icon-sm" disabled={isPending} onClick={() => handleDeletePlaylist(playlist.id)}>
      ...
      <div className="fixed bottom-6 right-6 flex flex-col gap-2 items-end">
        <AnimatePresence mode="wait">
          {activeTab === "groups" && (
            ...
            >
              <Button size="sm" variant="outline" className="rounded-full shadow-md gap-2" onClick={() => setJoinGroupOpen(true)}>
                <HugeiconsIcon icon={UserAdd01Icon} strokeWidth={2} />
                참가
              </Button>
              <Button size="sm" variant="outline" className="rounded-full shadow-md gap-2" onClick={() => setCreateGroupOpen(true)}>
          ...
          {activeTab === "shelf" && (
            <motion.div
              ...
              transition={{ duration: 0.15 }}
            >
              <Button size="icon-lg" className="rounded-full shadow-lg" onClick={() => setCreatePlaylistOpen(true)}>
